...
function gauge(num) {
  ...
  if (num <= 50000){          // 0 ~ 5만 구간
    ...
    num = num - 50000;
    sum = 141 * (num / 50000) + step[0];
  }else if (num <= 200000){   // 10만 ~ 20만 구간
    num = num - 100000;
    sum = 142 * (num / 100000) + step[0] + step[1];
  }else if (num <= 300000){   // 20만 ~ 30만 구간
    num = num - 200000;
    sum = 142 * (num / 100000) + step[0] + step[1] + step[2];
  }else{                      // 30만 이상 구간
    num = num - 300000;
    sum = 158 * (num / 200000) + step[0] + step[1] + step[2] + step[3];
    if (sum > $('#event .gauge .large').width() ) sum = '100%';
  }
...
$(document).ready(function(){
  ...
  $('#header .movie .btn_play').bind('click', function () {
    ...
    layerOpen('layer_movie', 'oUJX4OTQa3U');
  })
