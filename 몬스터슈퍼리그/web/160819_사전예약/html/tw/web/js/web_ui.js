...
function gauge(num) {
  ...
  if(num >= 50000 && num < 100000){
    $('#event').addClass('step1');
  }else if (num >= 100000 && num < 200000){
    $('#event').addClass('step2');
  }else if (num >= 200000 && num < 300000){
    $('#event').addClass('step3');
  }else if (num >= 300000 && num < 500000){
    $('#event').addClass('step4');
  }else if (num >= 500000){
    $('#event').addClass('step5');
  }
  ...
  $('#event .gauge .large span').width(sum);
  $('#event .gauge .small span').width(smallSum);
...
// clear input
function clearInp() {
  $('.inp_area input').removeClass('active').val('');
}
...
$(document).ready(function(){
  ...
  // input placeholder
  $('#regForm .mobile input, #regForm .mail input').focus(function(){
  ...
  $('#header .movie .btn_play').bind('click', function () {
    ...
    $(this).fadeOut(500);
    $(target).append('<iframe id="videoMain" class="video_yt" width="310" height="171" src="https://www.youtube.com/embed/aLsGp87oJ5s?&rel=0&;showinfo=0&;enablejsapi=1&;vq=hd720;wmode=transparent&autoplay=1" frameborder="0" allowfullscreen></iframe>');
  })
