/****************************************
 ...
 ver 1.2 (2017-05-22)
 - 입력 받은 값 연산 시 숫자로 강제 변환하는 과정 추가
 - 도착 지점의 data-reward값 리턴 값에 추가
  ...
  $.fn.Boardgame = function (obj ,loopNum, diceCount, diceScore) {
    ...
    Boardgame.defaultOps.target = $( this );
      ...
      var map = $(data.target).find('.map'),
        ...
        diceScore = (diceScore != undefined) ? parseInt(diceScore) : Math.floor((Math.random() * data.diceSet) + 1),
        startPos = parseInt($(activeMap).attr('data-step')),
        userPos = startPos,
        arrivePos = startPos + diceScore,
        typeMap = $(map).eq(arrivePos).attr('data-event'),
        reward = $(map).eq(arrivePos).attr('data-reward'),
    ...
    /* 4. 결과 처리 */
      ...
      returnData = {
        ...
        reward: reward                       // 도착 위치의 보상명(data-reward)
      };
