/*
 ...
 [ ftt.jquery.modallayer.js ]
   ...
   < history >
     ...
     ver 0.2
       - layerName 속성 추가(태그 data-name 속성으로 지정하며 레이어 오픈 시 class로 추가 됨)
     ver 0.3
       - 수동 출력 기능 추가
       - 일부 변수 명 변경
*/
...
;( function (factory) {
  ...
  var ModalLayer = (function(){
    function ModalLayer(ele, data) {
      ...
      _.openLayer = function (obj) {
        ...
        // display
        $(options.layerEle).addClass(obj.layerName).css(layerPos).show();
      ...
      _.setOpts = function (target) {
        ...
        if ( isElements ) {
          targetOpts = {
            ...
            layerName: ( $(target).attr('data-name') === undefined ) ? null : $(target).attr('data-name')
          };
        }else{
          targetOpts = {
            docHeight: $(document).height(),
            layerEle: ( target.layerEle !== undefined ) ? target.layerEle : _.defaultOpts.layerWrap + ' .' + target.layerType,
            viewerEle: $(_.defaultOpts.layerWrap + ' .' + target.layerType + ' .viewer div'),
            shareEle: $(_.defaultOpts.layerWrap).find('.video .share'),
            layerType: target.layerType,
            layerUrl: ( target.layerUrl !== undefined ) ? _.setList(target.layerUrl) : null ,
            layerMsg: ( target.layerMsg !== undefined ) ? _.setList(target.layerMsg) : null ,
            layerThumb: ( target.layerThumb !== undefined ) ? _.setList(target.layerThumb) : null,
            layerIdx: ( target.layerIdx !== undefined ) ? target.layerIdx : null,
            layerName: ( target.layerName !== undefined ) ? target.layerName : null,
            fixed: target.fixed
          }
        }
  ...
  ModalLayer.prototype.close = function (e) {
    ...
    if ( e.target.id != 'fLayer' ) {
      ...
      $(options.layerEle).removeClass(options.layerName).hide();
    }
  };

  $.fn.modalLayer = function ( data ) {
    var _ = this;

    _.modalLayer = new ModalLayer(_, data);
    return _;
  };
