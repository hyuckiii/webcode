 ...
 * mod1(17-04-25)
 * - 기능추가
 *   - sectionUp, sectionDown 시 slide연동 기능 추가
 *   - slide, scrolling 애니메이션 속도 분리 지정
 *
 * - 옵션 추가
 *   - options.slideSync: section 이동 시 slide 연동
 *   - options.slideSpeed: slide 애니메이션 속도 지정
 *
 */
(function(global, factory) {
    ...
    $.fn.fullpage = function(options) {
        ...
        function moveSectionUp(){
            ...
            var targetSlide = $(SECTION_ACTIVE_SEL).find('.fp-slides');
            var isSlide = ( targetSlide.length > 0 ) ? true : false,
              slidePos = $(targetSlide).find('.active').index();
            ...
            // mod - wheelSlide
            if ( isSlide &&  slidePos > 0 && options.slideSync ) {
                moveSlideLeft($(SECTION_ACTIVE_SEL));
                return false
            }
        ...
        function moveSectionDown(){
            ...
            var isSlide = ( targetSlide.length > 0 ) ? true : false,
              slidePos = $(targetSlide).find('.active').index(),
              slideNum = $(targetSlide).find('.slide').length - 1;
            ...
            // mod - wheelSlide
            if ( isSlide &&  slidePos < slideNum && options.slideSync ) {
                moveSlideRight($(SECTION_ACTIVE_SEL));
                return false
            }
        ...
        function performHorizontalMove(slides, v, fireCallback){
            ...
            if(options.css3){
                ...
                }, options.slideSpeed, options.easing);
            }else{
                slides.animate({
                    scrollLeft : Math.round(destinyPos.left)
                }, options.slideSpeed, options.easing, function() {
         ...
         * mod - slideSpeed 및 scrollingSpeed 각각 적용
         */
        function addAnimation(element){
            var aniSpeed = ( element.attr('class') === 'fp-slidesContainer' ) ? options.slideSpeed : options.scrollingSpeed;
            var transition = 'all ' + aniSpeed + 'ms ' + options.easingcss3;
