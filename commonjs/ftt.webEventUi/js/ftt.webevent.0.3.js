/****************************************
 ...
 0.2
 - setOtherItem 버그 수정
 - otherItem을 이용해 여러 항목내 동일한 내용을 넣을 수 있는 속성 추가(otherItem.once: ture)

 0.3
 - defaultOps.questStep 값 변경 ['', 'selected', 'ready', 'end']
...
(function(){
  ...
  var webEventUI = {
    defaultOps : {
      ...
      questStep: ['', 'selected', 'ready', 'end'],  // 퀘스트 단계별 클래스명
    ...
    setQuestList: function(data){
      $(data.target).each(function (i) {
        var quest = $(data.target[i]).find('li'+data.listEle),
          removeList = (data.questStep.toString()).replace(/,/gm, ' ') + ' disabled';
        ...
        if (data.target.length == 1){   // 미션(목록X)이 1개 인경우
          ...
          $(quest).each(function (j, questList) {
            var questStatus = data.questStep[data.questStatus[i][j]],
              ...
              maxVal = $(questList).find('.status .max'),
              title = $(questList).find('.quest-title'),
              rewardTitle = $(questList).find('.reward-title'),
            ...
            $(questList).removeClass(removeList).addClass(questStatus);
          })
        }
      });
    },
    setOtherItem: function (obj) {
      var target, value, once;
      ...
      for (var otherItems in obj.otherItem) {
        ...
        for (var itemsValue in obj.otherItem[otherItems]) {
          ...
          if (itemsValue == 'once') once = obj.otherItem[otherItems][itemsValue];
        }

        if($(target).length > 1 && once !== true){
  ...
  $.fn.webEventUI = function (obj, lang) {
    ...
    if (data.target.length > 0){
      ...
      // set otherItem
      if(data.otherItem != null) webEventUI.setOtherItem(data);
    }else{
